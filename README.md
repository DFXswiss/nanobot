...
| `TELEGRAM_GROUP_POLICY` | Optional: `mention` (default — only @mentions trigger the bot in group chats) or `open` (every group message triggers). DMs always work. For `open` to actually let the bot see non-@mention messages, the bot must either be an **administrator** in the group (admin status overrides privacy mode) or have BotFather privacy disabled (`/setprivacy` → Disable, then re-add the bot to the group). |
